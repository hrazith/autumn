jQuery(document).ready(function($) {
	function ProductBuilder(element) {
		...
		this.stepsWrapper = this.element.children('.qsections');
		this.steps = this.element.find('.qstep');
		...
		this.secondaryNavigation = this.element.find('.qmenu');
	...
	ProductBuilder.prototype.updateListOptions = function(listItem) {
		...
		if (listItem.hasClass('js-radio')) {
			...
			(listItem.hasClass('selected')) ? price = -Number(listItem.data('price')): price = Number(listItem.data('price')) + price;
	...
	ProductBuilder.prototype.customizeModel = function(target) {
		...
		//update final price
		var price = (parent.hasClass('selected')) ? 0 : Number(parent.data('price')) - parent.siblings('.selected').data('price');
